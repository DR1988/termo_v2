...
static void lcd_write(uint8_t data,uint8_t rs) 
{
    ...
    {
        /* configure data pins as output */
        DDR(LCD_DATC0_PORT) |= _BV(LCD_DATC0_PIN);
        DDR(LCD_DATC1_PORT) |= _BV(LCD_DATC1_PIN);
        DDR(LCD_DATC2_PORT) |= _BV(LCD_DATC2_PIN);
    	...
    	if(data & 0x80) LCD_DATC3_PORT |= _BV(LCD_DATC3_PIN);
    	if(data & 0x40) LCD_DATC2_PORT |= _BV(LCD_DATC2_PIN);
    	if(data & 0x20) LCD_DATC1_PORT |= _BV(LCD_DATC1_PIN);
    	if(data & 0x10) LCD_DATC0_PORT |= _BV(LCD_DATC0_PIN);   
        lcd_e_toggle();
        ...
        LCD_DATC1_PORT &= ~_BV(LCD_DATC1_PIN);
        LCD_DATC0_PORT &= ~_BV(LCD_DATC0_PIN);
    	if(data & 0x08) LCD_DATC3_PORT |= _BV(LCD_DATC3_PIN);
    	if(data & 0x04) LCD_DATC2_PORT |= _BV(LCD_DATC2_PIN);
    	if(data & 0x02) LCD_DATC1_PORT |= _BV(LCD_DATC1_PIN);
    	if(data & 0x01) LCD_DATC0_PORT |= _BV(LCD_DATC0_PIN);
        lcd_e_toggle();        
...
static uint8_t lcd_read(uint8_t rs) 
{
    ...
    else
    {
        ...
        lcd_e_delay();        
        data = 0;
        if ( PIN(LCD_DATC0_PORT) & _BV(LCD_DATC0_PIN) ) data |= 0x10;
        if ( PIN(LCD_DATC1_PORT) & _BV(LCD_DATC1_PIN) ) data |= 0x20;
        if ( PIN(LCD_DATC2_PORT) & _BV(LCD_DATC2_PIN) ) data |= 0x40;
        if ( PIN(LCD_DATC3_PORT) & _BV(LCD_DATC3_PIN) ) data |= 0x80;
        lcd_e_low();
        ...
        lcd_e_high();
        lcd_e_delay();
        if ( PIN(LCD_DATC0_PORT) & _BV(LCD_DATC0_PIN) ) data |= 0x01;
        if ( PIN(LCD_DATC1_PORT) & _BV(LCD_DATC1_PIN) ) data |= 0x02;
...
void lcd_init(uint8_t dispAttr)
{
    ...
    else if ( ( &LCD_DATC0_PORT == &LCD_DATC1_PORT) && ( &LCD_DATC1_PORT == &LCD_DATC2_PORT ) && ( &LCD_DATC2_PORT == &LCD_DATC3_PORT )
           && (LCD_DATC0_PIN == 0 ) && (LCD_DATC1_PIN == 1) && (LCD_DATC2_PIN == 2) && (LCD_DATC3_PIN == 3) )
    {
